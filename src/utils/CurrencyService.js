...
class LiveExchangeRateService {
  ...
  static async getLiveRates() {
    ...
    if (this.USE_STATIC_FALLBACK) {
      console.log('🔍 DEBUG: Using static rates (development mode enabled via VITE_USE_STATIC_RATES)');
    ...
    if (this.lastApiCall && (now - this.lastApiCall) < this.API_CALL_INTERVAL) {
      ...
      console.log(`⏳ DEBUG: Rate limiting active, waiting ${waitTime}s before API call`);
      ...
      if (this.cache.has('rates') && this.lastFetch && (now - this.lastFetch) < this.CACHE_DURATION) {
        ...
        console.log('🔍 DEBUG: Using cached rates during rate limiting:', cachedRates);
        return cachedRates;
      }

      console.log('📊 DEBUG: Falling back to static rates during rate limiting');
      return this.getStaticRates();
    }

    try {
      console.log('🌐 DEBUG: Fetching live exchange rates from API...');
      console.log('🔗 DEBUG: API URL:', import.meta.env?.VITE_EXCHANGE_RATE_API_URL || 'https://api.exchangerate-api.com/v4/latest/INR');
      ...
      if (rates && typeof rates === 'object' && rates.INR === 1) {
        ...
        console.log('✅ DEBUG: Successfully fetched live rates from API:', rates);
        console.log('📊 DEBUG: Live API rates are now active and cached');
        return rates;
      } else {
        console.warn('⚠️ DEBUG: Invalid API response, falling back to static rates');
        console.warn('⚠️ DEBUG: API Response:', rates);
        return this.getStaticRates();
      }
    } catch (error) {
      console.error('❌ DEBUG: API call failed, using static rates as fallback:', error.message);
      console.error('❌ DEBUG: Error details:', error);

      // Try cached rates first before falling back to static
      if (this.cache.has('rates') && this.lastFetch && (now - this.lastFetch) < this.CACHE_DURATION) {
        const cachedRates = this.cache.get('rates');
        console.log('🔍 DEBUG: Using cached rates as fallback after API failure:', cachedRates);
        return cachedRates;
      }

      console.log('📊 DEBUG: No cached rates available, using static rates as final fallback');
      return this.getStaticRates();
    }
  }
  ...
  static async fetchFromAPI() {
    ...
    if (primaryApiUrl) {
      console.log(`🔗 DEBUG: Trying primary exchange rate API: ${primaryApiUrl}`);
      try {
          ...
          if (data.data && typeof data.data === 'object') {
            ...
            for (const [currency, rateData] of Object.entries(data.data)) {
              if (CURRENCIES[currency] && rateData && typeof rateData.value === 'number' && rateData.value > 0) {
                ...
                console.log(`🔄 DEBUG: Transformed ${currency}: 1 ${currency} = ${transformedRates[currency]} INR`);
              }
            }

            console.log('✅ DEBUG: Successfully fetched from primary API:', transformedRates);
            return transformedRates;
          }
        } else {
          console.warn(`⚠️ DEBUG: Primary API failed with status ${response.status}: ${response.statusText}`);
        }
      } catch (error) {
        console.warn('⚠️ DEBUG: Primary API error:', error.message);
      }
    }

    // Try fallback API
    console.log(`🔗 DEBUG: Trying fallback exchange rate API: ${fallbackApiUrl}`);
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('📥 DEBUG: Fallback API response:', data);
        ...
        if (data.rates && typeof data.rates === 'object') {
          ...
          console.log('✅ DEBUG: Successfully fetched from fallback API:', transformedRates);
          return transformedRates;
        }
      }
      ...
      console.warn('⚠️ DEBUG: Fallback API failed, using static rates:', error.message);
      console.log('📊 DEBUG: Using updated static rates as reliable fallback');
  ...
  static getStaticRates() {
    ...
    if (this.cache.has('static_rates') && this.lastFetch && (now - this.lastFetch) < this.CACHE_DURATION) {
      console.log('🔍 DEBUG: Using cached static rates');
    ...
    console.log('📊 DEBUG: Using static rates as fallback:', staticRates);
    return staticRates;
  }
  ...
  static validateRate(apiRate, currency, fallbackRate) {
    ...
    if (isNaN(rate) || rate <= 0) {
      console.warn(`⚠️ DEBUG: Invalid API rate for ${currency}, using fallback: ${fallbackRate}`);
      return fallbackRate;
    }
    return rate;
  }
  ...
  static clearCache() {
    this.cache.clear();
    this.lastFetch = null;
    console.log('🔄 DEBUG: Exchange rate cache cleared');
  }
  ...
  static async forceRefresh() {
    console.log('🔄 DEBUG: Force refreshing exchange rates...');
    this.clearCache();
  ...
  static async testApiConnection() {
    try {
      ...
      console.log('✅ DEBUG: API test successful, sample rates:', testRates);
      return {
        success: true,
        rates: testRates,
  ...
  static async forceRefreshAndTest() {
    console.log('🔄 DEBUG: Force refreshing and testing API...');
    this.clearCache();
   ...
   static cleanup() {
     try {
       ...
       if (stats.hasRates && !stats.isFresh) {
         console.log('🧹 DEBUG: Cleaning up expired cache');
         this.clearCache();
       }
     } catch (error) {
...
export class CurrencyService {
  ...
  static async initialize() {
    console.log('🚀 DEBUG: Initializing CurrencyService with live rates...');
    try {
      await this.getExchangeRateWithRefresh('USD'); // This will trigger a fresh API call
      console.log('✅ DEBUG: CurrencyService initialized successfully with live rates');
    } catch (error) {
      console.warn('⚠️ DEBUG: Failed to initialize live rates, using static rates:', error.message);
    }
  }
  ...
  /**
    ...
    static async convertCurrency(amount, fromCurrency, toCurrency) {
      console.log(`🔄 DEBUG: CurrencyService.convertCurrency(${amount} ${fromCurrency} to ${toCurrency})`);
      ...
      if (fromCurrency === toCurrency) {
        console.log(`✅ DEBUG: Same currency, returning cleaned amount: ${cleanAmount}`);
        return cleanAmount;
      }
      ...
      try {
        ...
        console.log(`🔄 DEBUG: Converted to INR: ${amountInINR}`);

        // Then convert from INR to target currency
        const finalAmount = await this.convertINRToCurrency(amountInINR, toCurrency);
        console.log(`✅ DEBUG: Final converted amount: ${finalAmount}`);
        return finalAmount;
      } catch (error) {
        console.error(`❌ DEBUG: Error in convertCurrency:`, error);
  ...
  /**
      ...
      static formatCurrencyDisplay(amountINR, displayCurrency) {
        console.log(`🔍 DEBUG: CurrencyService.formatCurrencyDisplay(${amountINR} INR to ${displayCurrency})`);
        ...
        try {
          if (!isFinite(numAmount)) {
            console.log(`⚠️ DEBUG: Invalid amount for formatting: ${amountINR} (sanitized: ${numAmount})`);
            return "₹0.00";
          }

          const formatted = formatAmountInCurrency(numAmount, displayCurrency);
          console.log(`✅ DEBUG: Formatted amount: ${formatted}`);
          return formatted;
        } catch (error) {
          console.error('❌ DEBUG: Error formatting currency display:', error);
  ...
  static sanitizeAmount(input) {
    console.log(`🔍 DEBUG: sanitizeAmount input: "${input}" (type: ${typeof input})`);

    if (input === "" || input === null || input === undefined) {
      console.log(`✅ DEBUG: sanitizeAmount returning 0 for empty/null/undefined`);
      return 0;
    }

    // Convert to string and extract only valid numeric patterns
    const stringInput = String(input);
    console.log(`🔍 DEBUG: Converted to string: "${stringInput}"`);
    ...
    if (numberMatch) {
      ...
      console.log(`✅ DEBUG: Regex match found: "${extractedNumber}" → ${num}`);
      if (!isNaN(num) && isFinite(num)) {
        return Math.max(0, num); // Ensure non-negative
      }
    }

    console.log(`⚠️ DEBUG: No valid regex match found, trying aggressive cleaning`);

    // Aggressive fallback: remove all non-numeric characters except decimal point
    const cleaned = stringInput.replace(/[^0-9.]/g, '');
    console.log(`🔍 DEBUG: After aggressive cleaning: "${cleaned}"`);
    ...
    if (parts.length > 2) {
      ...
      console.log(`✅ DEBUG: Multiple decimals handled: "${integerPart}.${decimalPart}" → ${finalNum}`);
  ...
  static getCurrencySymbol(currencyCode) {
    if (!currencyCode || typeof currencyCode !== 'string') {
      console.warn(`⚠️ DEBUG: Invalid currency code for symbol: ${currencyCode}`);
      return '₹';
    }
  ...
  /**
   ...
   static getCurrencyName(currencyCode) {
     if (!currencyCode || typeof currencyCode !== 'string') {
       console.warn(`⚠️ DEBUG: Invalid currency code for name: ${currencyCode}`);
       return 'Indian Rupee';
     }
  ...
  /**
   ...
   static getCurrencyInfo(currencyCode) {
     if (!currencyCode || typeof currencyCode !== 'string') {
       console.warn(`⚠️ DEBUG: Invalid currency code for info: ${currencyCode}`);
  ...
  static getExchangeRate(currencyCode) {
    try {
      ...
      if (!currencyCode || typeof currencyCode !== 'string') {
        console.warn(`⚠️ DEBUG: Invalid currency code: ${currencyCode}`);
        return 1;
      }
      ...
      if (cachedRates && cachedRates[currencyCode] && cacheStats.isFresh) {
        console.log(`🔍 DEBUG: CurrencyService.getExchangeRate(${currencyCode}) = ${cachedRates[currencyCode]} (CACHED LIVE RATE)`);
      ...
      if (!cacheStats.isFresh || !cachedRates) {
        console.log(`🔄 DEBUG: Cache stale or empty for ${currencyCode}, triggering background refresh...`);
        // Trigger background refresh but don't wait for it
        LiveExchangeRateService.getLiveRates().catch(error => {
          console.warn(`⚠️ DEBUG: Background rate refresh failed for ${currencyCode}:`, error.message);
        });
      }

      const rate = STATIC_EXCHANGE_RATES[currencyCode] || 1;
      console.log(`🔍 DEBUG: CurrencyService.getExchangeRate(${currencyCode}) = ${rate} (STATIC RATE - using fallback)`);
      return rate;
    } catch (error) {
      console.warn(`⚠️ DEBUG: Failed to get rate for ${currencyCode}, using static:`, error.message);
  ...
  static async getExchangeRateWithRefresh(currencyCode) {
    try {
      ...
      console.log(`🔄 DEBUG: Force refreshing rates for ${currencyCode}...`);
      ...
      if (freshRates && freshRates[currencyCode]) {
        console.log(`✅ DEBUG: CurrencyService.getExchangeRateWithRefresh(${currencyCode}) = ${freshRates[currencyCode]} (FRESH LIVE RATE)`);
        return freshRates[currencyCode];
      }

      const rate = STATIC_EXCHANGE_RATES[currencyCode] || 1;
      console.log(`🔍 DEBUG: CurrencyService.getExchangeRateWithRefresh(${currencyCode}) = ${rate} (STATIC RATE - fallback)`);
      return rate;
    } catch (error) {
      console.warn(`⚠️ DEBUG: Failed to get rate for ${currencyCode}, using static:`, error.message);
      return STATIC_EXCHANGE_RATES[currencyCode] || 1;
  ...
  /**
   ...
   static isGSTApplicable(currencyCode) {
     if (!currencyCode || typeof currencyCode !== 'string') {
       console.warn(`⚠️ DEBUG: Invalid currency code for GST check: ${currencyCode}`);
       return false;
     }
  ...
  /**
   ...
   static shouldApplyInternationalTax(currencyCode, client) {
     // Validate currency code
     if (!currencyCode || typeof currencyCode !== 'string') {
       console.warn(`⚠️ DEBUG: Invalid currency code for international tax check: ${currencyCode}`);
       return false;
     }
  ...
  /**
   ...
   static calculateGST(amountINR, clientState, client = null) {
     console.log(`🔄 DEBUG: calculateGST called with amount: ${amountINR}, client: ${client?.country}, state: ${clientState}`);

     // Clean the input amount first
     const cleanedInput = String(amountINR).match(/[-+]?[0-9]*\.?[0-9]+/g)?.[0] || '0';
     const cleanAmountINR = Number(cleanedInput) || 0;

     console.log(`✅ DEBUG: calculateGST cleaned amount: ${amountINR} → ${cleanAmountINR}`);

     const isIndianClient = this.isIndianClient(client);
     console.log(`🔍 DEBUG: Is Indian client: ${isIndianClient}`);

     if (!isIndianClient) {
       // For international clients, apply 18% flat tax when invoicing in INR
       console.log('🌍 DEBUG: Applying international tax (18%) for non-Indian client');
       const taxRate = 18;
       ...
       console.log(`✅ DEBUG: International tax calculation result:`, result);
       return result;
     }
  ...
  /**
     ...
     static async calculateTotalWithTax(amountINR, currency, clientState, client = null) {
       console.log(`🔄 DEBUG: CurrencyService.calculateTotalWithTax(${amountINR} INR, ${currency}, ${clientState}, ${client?.country})`);

       // Clean the input amount first
       const cleanedInput = String(amountINR).match(/[-+]?[0-9]*\.?[0-9]+/g)?.[0] || '0';
       const cleanAmountINR = Number(cleanedInput) || 0;

       console.log(`✅ DEBUG: calculateTotalWithTax cleaned amount: ${amountINR} → ${cleanAmountINR}`);

       const isIndianClient = this.isIndianClient(client);
       const isLocalCurrency = client && this.getDefaultCurrencyForClient(client) === currency;

       console.log(`🔍 DEBUG: Is Indian client: ${isIndianClient}, Is local currency: ${isLocalCurrency}`);
       ...
       if (!isIndianClient && isLocalCurrency && currency !== 'INR') {
         ...
         console.log(`✅ DEBUG: International client local currency: subtotal=${subtotalDisplay}, total=${totalDisplay}, rate=${exchangeRate}`);
         return {
       ...
       console.log(`🔍 DEBUG: GST applicable: ${gstApplicable}, International tax applicable: ${internationalTaxApplicable}`);
       ...
       if (!gstApplicable && !internationalTaxApplicable) {
         ...
         const subtotalDisplay = await this.convertINRToCurrency(cleanAmountINR, currency);
         const totalDisplay = await this.convertINRToCurrency(cleanAmountINR, currency);
         console.log(`✅ DEBUG: No tax calculation: subtotal=${subtotalDisplay}, total=${totalDisplay}, rate=${exchangeRate}`);
         return {
           subtotalINR: cleanAmountINR,
           taxINR: 0,
           totalINR: cleanAmountINR,
       ...
       console.log(`🔍 DEBUG: Tax calculation result:`, gstResult);
       ...
       console.log(`✅ DEBUG: Tax calculation: subtotal=${subtotalDisplay}, tax=${taxDisplay}, total=${totalDisplay}, rate=${exchangeRate}`);
  ...
  static validateCurrencyForClient(currency, client) {
    if (!currency || typeof currency !== 'string') {
      console.warn(`⚠️ DEBUG: Invalid currency for validation: ${currency}`);
      return false;
    }
  ...
  static getDefaultCurrencyForClient(client) {
    if (!client?.country) {
      console.log('🔍 DEBUG: CurrencyService.getDefaultCurrencyForClient - No client country, returning INR');
      return 'INR';
    }
    ...
    console.log('🔍 DEBUG: CurrencyService.getDefaultCurrencyForClient:', {
  ...
  /**
     ...
     static async prepareInvoiceDataForStorage(formData, selectedCurrency, client) {
       console.log(`🔄 DEBUG: CurrencyService.prepareInvoiceDataForStorage - currency: ${selectedCurrency}, subtotal: ${formData.subtotal}, client: ${client?.country}`);
       ...
       console.log(`🔍 DEBUG: Subtotal in selected currency: ${formData.subtotal} → ${subtotalInSelectedCurrency}`);

       const subtotalINR = await this.convertCurrencyToINR(subtotalInSelectedCurrency, selectedCurrency);
       console.log(`🔍 DEBUG: Subtotal in INR: ${subtotalINR}`);
       ...
       console.log(`🔍 DEBUG: Tax calculation:`, gstCalculation);
       ...
       console.log(`🔍 DEBUG: Exchange rate: ${currentExchangeRate}, isLive: ${isLive}`);
       ...
       console.log(`✅ DEBUG: Display amounts - tax: ${taxAmountDisplay}, total: ${totalAmountDisplay}`);
  ...
  /**
   ...
   static async convertINRToCurrency(amountINR, targetCurrency) {
     try {
       ...
       if (!isFinite(numAmount)) {
         console.warn(`⚠️ DEBUG: Invalid amount for conversion: ${amountINR} (cleaned: ${cleanedInput})`);
         return 0;
       }

       if (!targetCurrency || !CURRENCIES[targetCurrency]) {
         console.warn(`⚠️ DEBUG: Invalid target currency: ${targetCurrency}`);
       ...
       if (!isFinite(convertedAmount)) {
         console.warn(`⚠️ DEBUG: Invalid conversion result: ${convertedAmount}`);
         return 0;
       }

       console.log(`🔍 DEBUG: convertINRToCurrency(${numAmount} INR to ${targetCurrency}) = ${convertedAmount} (rate: ${rate})`);
       return convertedAmount;
     } catch (error) {
       console.error(`❌ Error converting INR to ${targetCurrency}:`, error);
  ...
  /**
   ...
   static async convertCurrencyToINR(amount, sourceCurrency) {
     try {
       ...
       if (!isFinite(numAmount)) {
         console.warn(`⚠️ DEBUG: Invalid amount for conversion: ${amount} (cleaned: ${cleanedInput})`);
         return 0;
       }

       if (!sourceCurrency || !CURRENCIES[sourceCurrency]) {
         console.warn(`⚠️ DEBUG: Invalid source currency: ${sourceCurrency}`);
         return numAmount; // Return cleaned amount if currency invalid
       }
       ...
       if (!isFinite(amountInINR)) {
         console.warn(`⚠️ DEBUG: Invalid conversion result: ${amountInINR}`);
         return 0;
       }

       console.log(`🔍 DEBUG: convertCurrencyToINR(${numAmount} ${sourceCurrency} to INR) = ${amountInINR} (rate: ${rate})`);
       return amountInINR;
     } catch (error) {
       console.error(`❌ Error converting ${sourceCurrency} to INR:`, error);
  ...
  static formatAmountForPDF(amount, currency) {
    try {
      ...
      console.error('❌ Error formatting amount for PDF:', error);
