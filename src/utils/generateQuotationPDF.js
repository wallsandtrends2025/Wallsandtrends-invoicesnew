...
/**
 * Quotation PDF generator aligned with the shared Walls & Trends visual reference.
 */
...
export async function generateQuotationPDF(quotation, client) {
  if (!quotation) throw new Error('Document data is required for PDF generation');
  ...
  let currency = quotation?.currency || 'INR';
  ...
  const rawSubtotalINR = quotation?.subtotal_inr || quotation?.subtotal || 0;
  const rawSubtotal = quotation?.subtotal || 0;
  ...
  y += 6;
  const heading =
    quotation?.pdf_heading ||
    quotation?.quotation_heading ||
    quotation?.proforma_heading ||
    (quotation?.quotation_id ? 'Quotation' : 'Proforma Invoice');
  ...
  const numberLabel = quotation?.quotation_id ? 'Quotation Number:' : 'Proforma Number:';
  const titleLabel = quotation?.quotation_id ? 'Quotation Title:' : 'Proforma Title:';
  const dateLabel = quotation?.quotation_id ? 'Quotation Date:' : 'Proforma Date:';
  const totalLabel =
    quotation?.quotation_id || quotation?.proforma_id || quotation?.proforma_type
      ? 'Quotation Total Cost:'
      : 'Total Cost:';

  const docNumber =
    quotation?.quotation_id || quotation?.proforma_id || quotation?.invoice_id || 'N/A';
  const titleFromServices = deriveTitleFromServices(quotation);
  const docTitle =
    quotation?.quotation_title ||
    quotation?.proforma_title ||
    quotation?.invoice_title ||
    titleFromServices ||
    quotation?.service_name ||
    'N/A';
  const docDate = safeToDate(
    quotation?.quotation_date ||
      quotation?.proforma_date ||
  ...
  const servicesRows = buildServiceRowsWithCurrency(quotation, displayCurrency);
...
/* ---------------------- Other exports ---------------------- */
export async function generateTaxQuotationPDF(quotation, client) {
  const taxQuotation = {
    ...quotation,
    pdf_heading: 'Tax Quotation',
  };
  return await generateQuotationPDF(taxQuotation, client);
}

export async function testQuotationPDFFixes() {
  const testQuotation = {
    quotation_type: 'WT',
    quotation_id: 'WT2510QTN034',
    quotation_date: new Date('2025-10-25').toISOString(),
    quotation_title: 'Quotation for Nenu Ready',
    currency: 'INR',
    services: [
      {
        name: 'Teaser, Trailer',
  ...
  return await generateQuotationPDF(testQuotation, testClient);
}
...
export async function testWTXLogoDetection() {
  ...
  const wtQuotation = {
    quotation_type: 'WT',
    quotation_id: 'WT2510QTN001',
  };

  const wtxQuotation = {
    quotation_type: 'WTX',
    quotation_id: 'WTX2510QTN001',
  };

  const wtType = (wtQuotation?.quotation_type || '').toString().toUpperCase().trim();
  ...
  const wtxType = (wtxQuotation?.quotation_type || '').toString().toUpperCase().trim();
...
function deriveTitleFromServices(quotation) {
  const arr = Array.isArray(quotation?.services) ? quotation.services : [];
  const first = arr[0]?.name;
  if (Array.isArray(first)) return first.filter(Boolean).join(', ');
  return first || '';
}

function buildServiceRowsWithCurrency(quotation, displayCurrency) {
  const arr = Array.isArray(quotation?.services) ? quotation.services : [];
  if (arr.length) {
    return arr.map((s, i) => {
  ...
  const desc = String(quotation?.service_description || '');
  const amt = Number(quotation?.subtotal || 0);
  const formattedAmount = amt.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  return [['9983', nm, desc, formattedAmount]];
}

function calcSubtotalFromServices(quotation) {
  const arr = Array.isArray(quotation?.services) ? quotation.services : [];
  if (!arr.length) return Number(quotation?.subtotal || 0) || 0;
