// src/components/ProformaList.jsx
...
export default function ProformaList() {
  ...
  // ------------------ total count ------------------
  // We'll scan & count only proforma-like docs (ID includes "PRF").
  useEffect(() => {
    (async () => {
      try {
        ...
        const onlyProformas = allDocs.filter((p) => {
          const candidateId =
            p.proforma_id || p.quotation_id || p.id || "";
          return candidateId.includes("PRF");
        });
        setTotalCount(onlyProformas.length);
      } catch (err) {
  ...
  const runPageQuery = useCallback(
    async ({ direction = "first", jumpDoc = null, targetPage = null } = {}) => {
      ...
      try {
        ...
        // map & filter to only Proforma docs:
        const mapped = snap.docs.map((d) => ({
          id: d.id,
          ...d.data(),
          _snap: d,
        }));
        const onlyProformas = mapped.filter((p) => {
          const candidateId =
            p.proforma_id || p.quotation_id || p.id || "";
          return candidateId.includes("PRF");
        });

        setRows(onlyProformas);
  ...
  // initial load and on pageSize change
  useEffect(() => {
    ...
    return (
      <div className="flex flex-col items-end gap-2 ml-auto">
        ...
        <span className="text-sm text-gray-600">
          Showing <strong>{shownFrom || 0}</strong>–
                              ...
                              openMenuForId === p.id ? (
                                <RowActionsBubble
                                  ...
                                  onPreview={() => {
                                    ...
                                    // 🔁 PREVIEW PROFORMA ROUTE
                                    navigate(
                                      `/dashboard/proforma/${idToShow}`
                                    );
                                  }}
                                />
                              ) : null
