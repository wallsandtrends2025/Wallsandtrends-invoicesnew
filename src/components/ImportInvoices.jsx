  ...
  const handleImport = async () => {
    ...
    reader.onload = async (e) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const services = [];
        for (let i = 1; i <= 10; i++) {
          const name = row[headers.indexOf(`service${i}_name_csv`)] || '';
          const description = row[headers.indexOf(`service${i}_description`)] || '';
          const amount = parseFloat(row[headers.indexOf(`service${i}_amount`)] || 0);
          if (name) {
        ...
        if (!invoiceData.invoice_id || !invoiceData.client_id) {
          errors.push(`Invalid row ${i}: Missing Invoice ID or Client ID`);
          continue;
        }
      ...
      alert(`Imported ${count} invoices. Errors: ${errors.length}`);
    };
